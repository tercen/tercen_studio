...
storage.summary = function(){
  ...
  teams =  client$persistentService$findByKind('Team', useFactory=T)

  list = lapply(teams, function(team){

    projects = client$documentService$findProjectByOwnersAndCreatedDate(
      startKey=list(team$id,'2028'),
      endKey=list(team$id,''))
...
users = client$persistentService$findByKind('User', useFactory=T)

sapply(users, function(user) user$email)
sapply(users, function(user) user$isValidated)
sapply(users, function(user) user$createdDate$value)
